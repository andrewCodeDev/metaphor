...
All operations in Metaphor defer execution. No operation produces data at registration time. Each operation records a semantic description into the graph. Data materializes only on `.collect()`.
...
```c3
// These lines only build the graph; no computation yet
...
- **Device-specific optimization**: the same graph produces different code per backend
...
If leaf data has not changed since the last execution, `.collect()` returns immediately.

## Execution Order

Execution proceeds via depth-first post-order traversal of dependencies. Diamond patterns (where two paths converge on the same tensor) compute each tensor exactly once. Intermediate memory is reclaimed automatically through reference counting as consumers finish.
