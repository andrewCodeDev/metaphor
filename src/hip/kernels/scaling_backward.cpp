...
//
// Appends to scaling.cpp - implements gradients for all scaling operations
...
// Min-max scaling backward
void hip_min_max_scale_backward_f32(const float *grad_y, float *grad_x,
...
void hip_zscore_backward_f32(const float *grad_y, float *grad_x, size_t n,
                              float std) {
  int threads = 256;
  int blocks = (n + threads - 1) / threads;
...
// L2 normalize backward
void hip_l2_normalize_backward_f32(const float *grad_y, const float *y,
                                    float *grad_x, size_t n, float norm) {
  // Compute dot(y, grad_y)
  float *d_dot;
  HIP_ASSERT(hipMalloc(&d_dot, sizeof(float)));
  HIP_ASSERT(hipMemset(d_dot, 0, sizeof(float)));
  ...
  float h_dot;
  HIP_ASSERT(hipMemcpy(&h_dot, d_dot, sizeof(float), hipMemcpyDeviceToHost));
  ...
  HIP_ASSERT(hipFree(d_dot));
}

// L1 normalize backward
void hip_l1_normalize_backward_f32(const float *grad_y, const float *x,
                                    float *grad_x, size_t n, float norm) {
  // Compute sum(grad_y * sign(x))
  float *d_sum;
  HIP_ASSERT(hipMalloc(&d_sum, sizeof(float)));
  HIP_ASSERT(hipMemset(d_sum, 0, sizeof(float)));
  ...
  HIP_ASSERT(hipFree(d_sum));
}

// Max-abs scale backward
void hip_max_abs_scale_backward_f32(const float *grad_y, const float *x,
...
// Clamp backward
void hip_clamp_backward_f32(const float *grad_y, const float *x, float *grad_x,
                             size_t n, float min_val, float max_val) {
  int threads = 256;
  int blocks = (n + threads - 1) / threads;
