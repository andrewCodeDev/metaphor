/*
 * memory/concat_stack.cpp - Concat and Stack operations for HIP
 *
...
struct ConcatParams {
    ...
    len_t base_shape[HIP_MAX_DIMS];       // Shape of first input (reference for non-concat dims)
...
extern "C" void hip_concat(
    ...
    LOG_DEBUG("hip_concat: num_inputs=%lu concat_dim=%lu ndim=%lu numel=%lu",
    ...
    DISPATCH_DTYPE(dtype,
        kernel_concat<scalar_t><<<num_blocks, block_size, 0, stream>>>(
            static_cast<scalar_t*>(out->data), params));
}
...
extern "C" void hip_stack(
    ...
    LOG_DEBUG("hip_stack: num_inputs=%lu stack_dim=%lu in_ndim=%lu numel=%lu",
    ...
    DISPATCH_DTYPE(dtype,
        kernel_stack<scalar_t><<<num_blocks, block_size, 0, stream>>>(
            static_cast<scalar_t*>(out->data), params));
}
