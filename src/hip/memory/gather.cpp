/*
 * memory/gather.cpp - Index-based gather operation for HIP
 *
...
__global__ void kernel_scatter_add(
    ...
    typed_atomic_add(&out[out_offset], grad[i]);
}
...
extern "C" void hip_gather(
    ...
    LOG_DEBUG("hip_gather: gather_dim=%lu in_ndim=%lu idx_ndim=%lu out_ndim=%lu numel=%lu",
    ...
    DISPATCH_DTYPE(dtype,
        kernel_gather<scalar_t, int64_t><<<num_blocks, block_size, 0, stream>>>(
            static_cast<scalar_t*>(out->data),
            static_cast<const scalar_t*>(in->data),
            static_cast<const int64_t*>(indices->data), params));
}
...
extern "C" void hip_scatter_add(
    ...
    LOG_DEBUG("hip_scatter_add: gather_dim=%lu out_ndim=%lu idx_ndim=%lu grad_ndim=%lu grad_numel=%lu",
    ...
    DISPATCH_DTYPE(dtype,
        kernel_scatter_add<scalar_t, int64_t><<<num_blocks, block_size, 0, stream>>>(
            static_cast<scalar_t*>(out->data),
            static_cast<const scalar_t*>(grad->data),
            static_cast<const int64_t*>(indices->data), params));
}
