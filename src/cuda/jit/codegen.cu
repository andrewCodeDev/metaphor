/*
 * jit/codegen.cu - CUDA kernel source code generation
 *
...
/* F16 binary op expressions using CUDA half intrinsics (SM 5.3+ required) */
...
static const char *binary_op_expr_bf16(BinOp op) {
  ...
  case BINOP_LTE:
    return "(__bfloat162float(a) <= __bfloat162float(b) ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
  case BINOP_AND:
    return "((__bfloat162float(a) != 0 && __bfloat162float(b) != 0) ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
  case BINOP_OR:
    return "((__bfloat162float(a) != 0 || __bfloat162float(b) != 0) ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
  case BINOP_XOR:
    return "(((__bfloat162float(a) != 0) != (__bfloat162float(b) != 0)) ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
 ...
 *
 * CUDA provides half-precision math intrinsics (hsqrt, hexp, hlog, hsin, hcos,
 ...
 * 3. Writing specialized CUDA kernels for critical operations
 *
 * Current implementation: Uses CUDA half intrinsics directly, requires SM 5.3+.
 * BF16 intrinsics require SM 8.0+ (Ampere) for native support.
 */

/* F16 unary operations using CUDA half intrinsics (SM 5.3+ required) */
...
static const char *unary_op_expr_bf16(MapOp op) {
  ...
  case MAP_SQR:
    return "(__float2bfloat16(__bfloat162float(x) * __bfloat162float(x)))";
  case MAP_SQRT:
  ...
  case MAP_RELU:
    return "(__bfloat162float(x) > 0 ? x : __float2bfloat16(0.0f))";
  case MAP_HEAVISIDE:
    return "(__bfloat162float(x) > 0 ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
  case MAP_SIGMOID:
    return "(__float2bfloat16(1.0f / (1.0f + expf(-__bfloat162float(x)))))";
  case MAP_GELU:
    return "(__float2bfloat16(0.5f * __bfloat162float(x) * (1.0f + tanhf(0.7978845608f * (__bfloat162float(x) + 0.044715f * __bfloat162float(x) * __bfloat162float(x) * __bfloat162float(x))))))";
  case MAP_SILU:
    return "(__float2bfloat16(__bfloat162float(x) / (1.0f + expf(-__bfloat162float(x)))))";
  case MAP_SOFTPLUS:
    return "(__float2bfloat16(fmaxf(__bfloat162float(x), 0.0f) + logf(1.0f + expf(-fabsf(__bfloat162float(x))))))";
  case MAP_NOT:
    return "(__bfloat162float(x) == 0 ? __float2bfloat16(1.0f) : __float2bfloat16(0.0f))";
  case MAP_SIGN:
    return "(__bfloat162float(x) > 0 ? __float2bfloat16(1.0f) : (__bfloat162float(x) < 0 ? __float2bfloat16(-1.0f) : __float2bfloat16(0.0f)))";
  default:
    return "x";
  }
}
