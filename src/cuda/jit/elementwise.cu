/*
 * jit/elementwise.cu - Elementwise kernel execution via JIT
 *
 * Extern C API for Zig to execute JIT-compiled elementwise operations.
 */
 ...
 */
extern "C" void jit_elementwise_unary(CudaDeviceHandle device_handle,
  ...
  if (same_shape) {
    ...
    len_t in_strides[CUDA_MAX_DIMS] = {0};
 ...
 */
extern "C" void jit_elementwise_binary(CudaDeviceHandle device_handle,
  ...
  if (same_shape) {
    ...
    len_t a_strides[CUDA_MAX_DIMS];
    len_t b_strides[CUDA_MAX_DIMS];
 ...
 */
extern "C" void jit_elementwise_select(CudaDeviceHandle device_handle,
  ...
  if (same_shape) {
    ...
    len_t mask_strides[CUDA_MAX_DIMS];
    len_t true_strides[CUDA_MAX_DIMS];
    len_t false_strides[CUDA_MAX_DIMS];
 ...
 */
extern "C" void jit_elementwise_inplace_chain(CudaDeviceHandle device_handle,
                                              DenseCore *out,
  ...
  if (same_shape) {
    ...
    /* Broadcasting case - compute strides for all inputs */
    len_t base_strides[CUDA_MAX_DIMS];
    len_t child_strides[MAX_INPLACE_CHAIN_OPS][CUDA_MAX_DIMS];
    ...
    len_t shape_vals[CUDA_MAX_DIMS];
    len_t stride_vals[MAX_INPLACE_CHAIN_OPS + 1][CUDA_MAX_DIMS];
    ...
    void *args[(MAX_INPLACE_CHAIN_OPS + 1) * CUDA_MAX_DIMS + MAX_INPLACE_CHAIN_OPS + 10];
    size_t arg_idx = 0;
 ...
 * ============================================================================
 */
extern "C" void jit_elementwise_fused_chain(CudaDeviceHandle device_handle,
                                            DenseCore *out,
  ...
  if (same_shape) {
    ...
    auto check_cap = [&](const char* label) {
      ...
      cudaStreamGetCaptureInfo(stream, &cap_status, nullptr);
    ...
    /* Broadcasting case - compute strides for all inputs */
    len_t input_strides[MAX_FUSED_CHAIN_OPS + 1][CUDA_MAX_DIMS];
    ...
    len_t shape_vals[CUDA_MAX_DIMS];
    len_t stride_vals[MAX_FUSED_CHAIN_OPS + 1][CUDA_MAX_DIMS];
    for (size_t d = 0; d < ndim; d++) {
      shape_vals[d] = out->shape.buffer[d];
    }
    for (size_t j = 0; j < num_inputs; j++) {
    ...
    void *args[(MAX_FUSED_CHAIN_OPS + 1) * CUDA_MAX_DIMS + MAX_FUSED_CHAIN_OPS + 10];
    size_t arg_idx = 0;
 ...
 *
 * Compiles and launches a pre-generated CUDA kernel source string.
 ...
 * ============================================================================
 */
extern "C" void jit_elementwise_from_source(CudaDeviceHandle device_handle,
                                            DenseCore *out,
                                            const DenseCore *inputs,
                                            size_t num_inputs,
                                            const char *source,
  ...
  void *args[34 + CUDA_MAX_DIMS]; /* max 32 inputs + out + n + dims */
  size_t arg_idx = 0;
  args[arg_idx++] = &out_ptr;
  ...
  size_t dim_sizes[CUDA_MAX_DIMS];
  if (needs_broadcast) {
    for (size_t d = 0; d < out->shape.len && d < CUDA_MAX_DIMS; d++) {
...
/*
 * Compile CUDA source to PTX without executing.
 * Returns heap-allocated PTX string (caller must free with jit_free_ptx).
 * Sets *ptx_size_out to the PTX size.
 * Returns nullptr on compilation failure.
 */
extern "C" const char* jit_compile_to_ptx(
    const char* source,
    size_t source_len,
    size_t* ptx_size_out
) {
    ...
    /* Copy PTX to a new allocation for the caller */
    char* ptx_copy = (char*)malloc(kernel->ptx_size);
    memcpy(ptx_copy, kernel->ptx, kernel->ptx_size);
    *ptx_size_out = kernel->ptx_size;
...
/*
 * Free PTX returned by jit_compile_to_ptx.
 */
extern "C" void jit_free_ptx(const char* ptx) {
    free(const_cast<char*>(ptx));
}

/*
 * Execute a kernel from cached PTX (no nvrtc compilation).
 * Same signature as jit_elementwise_from_source but takes PTX instead of CUDA source.
 */
extern "C" void jit_elementwise_from_ptx(
    CudaDeviceHandle device_handle,
    DenseCore* out,
    const DenseCore* inputs,
    size_t num_inputs,
    const char* ptx,
    ...
    LOG_DEBUG("jit_elementwise_from_ptx: num_inputs=%zu out_numel=%lu ptx_size=%zu",
              num_inputs, out->num_elements, ptx_size);

    if (ptx == nullptr || ptx_size == 0) {
        LOG_ERROR("jit_elementwise_from_ptx: null or empty PTX");
        return;
    }

    CUstream stream = static_cast<CUstream>(stream_handle.ptr);

    /* Load module directly from PTX */
    CUmodule module;
    CUresult err = cuModuleLoadData(&module, ptx);
    if (err != CUDA_SUCCESS) {
        const char* errStr;
        cuGetErrorString(err, &errStr);
        LOG_ERROR("jit_elementwise_from_ptx: cuModuleLoadData failed: %s", errStr);
        return;
    }

    CUfunction function;
    err = cuModuleGetFunction(&function, module, "kernel");
    if (err != CUDA_SUCCESS) {
        const char* errStr;
        cuGetErrorString(err, &errStr);
        LOG_ERROR("jit_elementwise_from_ptx: cuModuleGetFunction failed: %s", errStr);
    ...
    cuLaunchKernel(function, grid_size, 1, 1, block_size, 1, 1, 0, stream, args, nullptr);
...
/* ============================================================================
 * Handle-based PTX API for C3-driven codegen
 *
...
extern "C" void* jit_load_ptx(
    ...
    if (err != CUDA_SUCCESS) {
        const char* errStr;
        cuGetErrorString(err, &errStr);
        LOG_ERROR("jit_load_ptx: cuModuleLoadData failed: %s", errStr);
        return nullptr;
    }
    ...
    if (err != CUDA_SUCCESS) {
        const char* errStr;
        cuGetErrorString(err, &errStr);
        LOG_ERROR("jit_load_ptx: cuModuleGetFunction failed: %s", errStr);
        cuModuleUnload(module);
        return nullptr;
    }
...
extern "C" void jit_launch_kernel(
    ...
    cuLaunchKernel(k->function, grid_size, 1, 1, block_size, 1, 1, 0, stream, args, nullptr);
}
