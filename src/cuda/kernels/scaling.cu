// Scaling and Normalization Kernels for CUDA
//
...
// Min-max scaling
void cuda_min_max_scale_f32(float *x, float *y, size_t n, float min_val,
...
void cuda_min_max_scale_range_f32(float *x, float *y, size_t n, float min_val,
...
// Z-score
void cuda_zscore_f32(float *x, float *y, size_t n, float mean, float std,
                     float epsilon) {
  int threads = 256;
  int blocks = (n + threads - 1) / threads;
...
// L2 normalize
void cuda_l2_normalize_f32(float *x, float *y, size_t n, float epsilon) {
  // Compute norm
  float *d_norm;
  ...
  float h_norm_sq;
  cudaMemcpy(&h_norm_sq, d_norm, sizeof(float), cudaMemcpyDeviceToHost);
...
// L1 normalize
void cuda_l1_normalize_f32(float *x, float *y, size_t n, float epsilon) {
  float *d_norm;
  cudaMalloc(&d_norm, sizeof(float));
  cudaMemset(d_norm, 0, sizeof(float));
...
// Max-abs scale
void cuda_max_abs_scale_f32(float *x, float *y, size_t n, float epsilon) {
  float *d_max;
...
// Clamp
void cuda_clamp_f32(float *x, float *y, size_t n, float min_val,
                    float max_val) {
  int threads = 256;
  int blocks = (n + threads - 1) / threads;
