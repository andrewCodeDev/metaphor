/*
 * nn/dropout.cu - Inverted dropout forward and backward (CUDA)
 *
...
template<> __device__ __forceinline__ float to_float(bf16 x)   { return __bfloat162float(x); }
...
template<> __device__ __forceinline__ bf16   from_float(float x)  { return __float2bfloat16(x); }
...
extern "C" void cuda_nn_dropout_fwd(
    Dtype dtype,
...
extern "C" void cuda_nn_dropout_bwd(
    Dtype dtype,
    StreamHandle stream_handle,
    const void* y_grd,
    const void* mask,
