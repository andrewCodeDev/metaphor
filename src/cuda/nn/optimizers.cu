/*
 * nn/optimizers.cu - Optimizer update kernels
 *
 * CUSTOM KERNEL REQUIREMENTS FOR F16/BF16:
 *
 * These optimizers use Thrust templates with standard arithmetic operators,
 * which do NOT work directly with __half or __nv_bfloat16 types. Supporting
 * F16/BF16 would require:
 *
 * Option 1: Custom CUDA kernels using half intrinsics (__hadd, __hmul, etc.)
...
extern "C" void cuda_optim_momentum(
    Dtype id,
    StreamHandle w,
    void* param,
    const void* grad,
...
extern "C" void cuda_optim_rmsprop(
    Dtype id,
    StreamHandle w,
    void* param,
    const void* grad,
...
extern "C" void cuda_optim_adadelta(
    Dtype id,
    StreamHandle w,
    void* param,
    const void* grad,
...
struct AdamFunctor {
    ...
    __device__ thrust::tuple<T, T, T> operator()(
        ...
        T v_hat = new_v * bc2;
        // Update param
        T new_param = param - lr * m_hat / (sqrt(v_hat) + epsilon);
        return thrust::make_tuple(new_param, new_m, new_v);
    }
};

extern "C" void cuda_optim_adam(
    Dtype id,
    StreamHandle w,
    void* param,
    const void* grad,
    ...
    switch (id) {
        ...
        case DTYPE_F64: {
            ...
            thrust::transform(
                thrust::cuda::par.on(stream),
                in, in + n, out,
                AdamFunctor<f64>{lr, beta1, beta2, epsilon, bc1, bc2}
            );
            return;
        }
        default:
...
extern "C" void cuda_optim_sgd_master(
    Dtype visible_dtype,
    StreamHandle w,
...
__global__ void adam_master_kernel(
    ...
    if (i < n) {
        ...
        // Update master weights
        master[i] -= lr * m_hat / (sqrtf(v_hat) + epsilon);
...
extern "C" void cuda_optim_adam_master(
    Dtype visible_dtype,
    StreamHandle w,
    void* visible,          // F16/BF16 visible weights
    void* master,           // F32 master weights
...
extern "C" void cuda_optim_init_master(
    Dtype visible_dtype,
    StreamHandle w,
...
extern "C" void cuda_optim_clip_grad_norm(
    Dtype id,
    StreamHandle w,
    void* grad,
    len_t n,
 ...
 * 1. cuda_optim_grad_norm_sq: Compute sum of squares for one gradient tensor
 * 2. cuda_optim_scale_grad: Scale gradients by a pre-computed factor
 *
 * Usage for global clipping:
 *   total_sq = 0
 *   for each param: total_sq += cuda_optim_grad_norm_sq(grad)
 *   total_norm = sqrt(total_sq)
 *   if total_norm > max_norm: scale = max_norm / total_norm
 *   for each param: cuda_optim_scale_grad(grad, scale)
 * ============================================================================ */

// Compute sum of squares for a gradient tensor (returns to host)
extern "C" f32 cuda_optim_grad_norm_sq(
    Dtype id,
    StreamHandle w,
    const void* grad,
    len_t n
...
extern "C" void cuda_optim_scale_grad(
    Dtype id,
    StreamHandle w,
    void* grad,
    len_t n,
...
/* ============================================================================
 * Fused Clipped Adam (No Host Sync)
 *
 * Two-kernel approach that avoids DtoH transfer for gradient clipping:
 *   Kernel 1: Reduce grad² to a device-side float (norm_sq_out)
 *   Kernel 2: Read device norm, clip inline, then do Adam update
 *
 * Both run on same stream, so kernel 2 sees kernel 1's output without explicit sync.
 * ============================================================================ */

/* --------------------------------------------------------------------------
 * Kernel 1: Block-reduce grad² → atomicAdd into device-side float
 *
 * Uses shared memory tree reduction within each block, then a single
 * atomicAdd per block to accumulate into *norm_sq_out.
 * Caller must zero *norm_sq_out before launch (cudaMemsetAsync).
 * -------------------------------------------------------------------------- */
...
__global__ void adam_clipped_kernel(
    ...
    const float* __restrict__ norm_sq_ptr,  // device-side norm²
    len_t n,
    float lr,
    float beta1,
    float beta2,
    ...
    float max_norm     // clip threshold
) {
    len_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;

    // Read grad and apply clip scale
    float g = static_cast<float>(grad[i]);
...
extern "C" void cuda_optim_grad_norm_sq_device(
    Dtype dtype,
    StreamHandle stream_handle,
    const void* grad,
    void* norm_sq_out,      // device float* — must be zeroed before call
    len_t n
) {
...
extern "C" void cuda_optim_adam_clipped(
    Dtype dtype,
    StreamHandle stream_handle,
    void* param,
    const void* grad,
    void* m,
    void* v,
    const void* norm_sq_ptr,    // device float* with pre-computed norm²
    len_t n,
    f64 lr,
    f64 beta1,
    f64 beta2,
