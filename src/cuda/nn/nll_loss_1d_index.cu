/*
 * nn/nll_loss_1d_index.cu - Negative log likelihood loss with index targets
 */
...
static void nll_loss_1d_index_reduce(
    ...
    cudaStream_t stream = stream_from_cudnn(w);
...
extern "C" void cuda_nn_nll_loss_1d_index_fwd(
    Dtype id,
    CudnnHandle w,
    void* src,
    len_t trg,
    ...
    if (inplace_smax) {
        cuda_nn_smax_vec_fwd(id, w, src, src, n, SMAX_LOG);
    }
...
extern "C" void cuda_nn_nll_loss_1d_index_bwd(
    Dtype id,
    CudnnHandle w,
    const void* x_val,
    void* x_grd,
