/*
 * handles/properties.cu - Device property queries
 */
 ...
 *
 * These mirror cudaDeviceProp but use len_t for Zig compatibility.
 * ============================================================================ */
...
struct DeviceProperties {
    ...
    static DeviceProperties from_cuda(const cudaDeviceProp& prop) {
...
extern "C" unsigned cuda_device_count() {
    CUDRIVER_ASSERT(cuInit(0));
    int count;
    CUDRIVER_ASSERT(cuDeviceGetCount(&count));
    return static_cast<unsigned>(count);
}

extern "C" DevicePropertiesHandle cuda_device_properties(unsigned device_id) {
    cudaDeviceProp prop;
    CUDA_ASSERT(cudaGetDeviceProperties(&prop, device_id));

    auto* properties = new DeviceProperties(DeviceProperties::from_cuda(prop));
    return wrap_props(properties);
}

extern "C" void cuda_device_properties_free(DevicePropertiesHandle handle) {
    delete unwrap_props(handle);
}

extern "C" len_t cuda_device_total_memory(unsigned device_id) {
    len_t total;
    CUDRIVER_ASSERT(cuDeviceTotalMem(&total, device_id));
    return total;
}

/* ============================================================================
 * Property Accessors (for Zig to read individual fields)
 * ============================================================================ */

extern "C" len_t cuda_props_multi_processor_count(DevicePropertiesHandle h) {
    return unwrap_props(h)->multi_processor_count;
}

extern "C" len_t cuda_props_max_threads_per_block(DevicePropertiesHandle h) {
    return unwrap_props(h)->max_threads_per_block;
}

extern "C" len_t cuda_props_max_threads_per_mp(DevicePropertiesHandle h) {
    return unwrap_props(h)->max_threads_per_multi_processor;
}

extern "C" len_t cuda_props_warp_size(DevicePropertiesHandle h) {
    return unwrap_props(h)->warp_size;
}

extern "C" len_t cuda_props_shared_mem_per_block(DevicePropertiesHandle h) {
    return unwrap_props(h)->memory.shared_per_block;
}

extern "C" len_t cuda_props_total_global_mem(DevicePropertiesHandle h) {
    return unwrap_props(h)->memory.total_global;
}

extern "C" len_t cuda_props_compute_major(DevicePropertiesHandle h) {
    return unwrap_props(h)->compute_version.major;
}

extern "C" len_t cuda_props_compute_minor(DevicePropertiesHandle h) {
...
extern "C" void cuda_check_last_error() {
